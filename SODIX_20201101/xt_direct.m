function [relative_ang] = xt_direct(x_t, mic_info, mic_rang, mic_ang)
%ªªÀ„∑ΩœÚ
...
relative_ang = zeros(num_source,size(mic_info,1));
length_sou_mic = zeros(num_source,size(mic_info,1));
cos_ang = zeros(num_source,size(mic_info,1));
for s=1:num_source
    if x_t(s,1)==0
