function [CSM, freqs] = developCSM(x, freq_l, freq_u, Fs, t_block, overlap, SNR, t_start, t_end)
% function [CSM, freqs] = developCSM(x, freq_l, freq_u, Fs, t_block, overlap, SNR, t_start, t_end)
%     SNR=0;
...
x_fr = Fs / block_samples * (0:floor(block_samples/2)-1);%频率分度
...
%      x_fft11=fftshift(x_fft);
%      figure;plot(imag(x_fft11(5712:5714,1)))
%      figure;plot(imag(x_fft(200:202,1)))
%      figure;plot(imag(x_fft11(200:202,1)))
%      figure;plot(imag(x_fft11(21850:21852,1)))
%      figure;plot(imag(x_fft(200:202,1)))
%      figure;plot(imag(x_fft(21850:21852,1)))
     if( nargin == 7)
         %Add noise for the simulation data---nfl 2020.5.23
         x_fftnoise = AddNoise(x_fft, SNR);
     else
         x_fftnoise = x_fft;
     end
%      [Pxy,f]=csd(x,y,Nfft,Fs,window,Noverlap,'dflag');
    for F = 1:N_freqs
        CSM(:,:,F) = CSM(:,:,F) + 0.5*(x_fftnoise(freq_sels(F),:)'*x_fftnoise(freq_sels(F),:));
    end
