...
source_info_loc = [-1.320 0.04 1.06 2];%源位置[-5.4 -2.2 0 2]
...
[source_info_1] = source_setup(num_source_class, source_info_loc,source_info_num, 120, wavelength, x_range);
...
for i=1:size(source_info,1)
    [p1, Fs] = simulateArraydata_2(source_info(i,:), mic_pos, coherent, c, fs, t_sampling, [0 0 0],direct_amp(i,:)); 
    p = p+p1;
end
...
[fun_syms, x, dfun_syms,djm0] = obfunction_4(num_syms_x, x_t, CSM, mic_pos, freqs, ...
    z_range, c);
...
hold on
plot([source_info_1([1,4,7,10,end],1) source_info_1([1,4,7,10,end],1)], get(gca, 'YLim'), '-r')
hold on
