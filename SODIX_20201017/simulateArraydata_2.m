...
f_min = min(source_info(:, 4));% 频率min
f_max = max(source_info(:, 4));

% Array center
x_ac = mean(mic_info,1);  %无法计算阵列中心声压1弧形阵列2：指向性
...
%         bb=((r_max-r_ac)>0);???   ((r_ac-r_min>0)) hhj--20200528
        ...
        % 生成阵列中心处的指定声压级的白噪声 hhj--20200528
        for J = 1:N_mic
